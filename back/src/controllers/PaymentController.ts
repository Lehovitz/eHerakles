...
export default class PaymentController {
  ...
  async createWithCustId(req: Request, res: Response) {
    ...
    if(cust)
    {
      ...
      where: { customer: cust, paymentDate:paymentDate, status:status, period:period},
    });
    }
    ...
    if (!payment) {
      ...
      let card = await cardRepo.findOne({ customer : cust});
      if(card)
      {
        card.due = parseFloat(card.due.toString()) + parseFloat(due);
  ...
  async acceptPayment(req: Request, res: Response) {
    const repo = getManager().getRepository(Payment);
    let payment: Payment = await repo
    .createQueryBuilder("payment")
    .leftJoinAndSelect("payment.customer", "customer")
    .where("payment.id = :id", { id: req.params.id  })
    .getOne();
    ...
    if (payment) {
      let card = await cardRepo.findOne({customer: payment.customer})
      payment.status = Status.Finalized;
      console.log("zaktualizowano platnosc");
      await repo.save(payment);
      if(card)
      {
        card.due = parseFloat(card.due.toString()) - parseFloat(payment.due.toString());
        if(payment.period){
        card.expDate = addMonths(card.expDate, payment.period);

        }
        // let pendingPayments = await repo.find({where: {status: Not(Status.Finalized), customer: payment.customer}});
        // console.log(pendingPayments);
        // if(pendingPayments.length > 0)
        // {
        //   card.isActive = false;
        // }
        if(card.expDate<new Date()) {
          card.isActive=false;
        }
        else {
          card.isActive = true;
